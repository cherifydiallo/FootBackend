...
-- Création du type ENUM pour les rôles utilisateur (idempotent)
DO $$
BEGIN
...
CREATE INDEX IF NOT EXISTS idx_users_identifiant ON users(identifiant);
...
CREATE TABLE IF NOT EXISTS rest_permission_config (
    id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_rest_permission_key ON rest_permission_config(permission_key);
CREATE INDEX IF NOT EXISTS idx_rest_permission_group_id ON rest_permission_config(group_id);
...
CREATE INDEX IF NOT EXISTS idx_academies_name ON academies(academy_name);
...
CREATE INDEX IF NOT EXISTS idx_academy_categories_academy_id ON academy_categories(academy_id);
...
CREATE INDEX IF NOT EXISTS idx_players_register_number ON players(register_number);
